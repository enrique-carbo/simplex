...
export const POST: APIRoute = async ({ params, request, locals }) => {
  try {
    ...
    if (order.status !== 'draft' && order.status !== 'pending') {
      return new Response(JSON.stringify({ error: 'La orden no puede ser modificada' }), { status: 400 });
    }
    ...
    const numero = formData.get('shipping_address[numero]') as string;
    ...
    if (receiptFile && receiptFile.size > 0 && receiptFile.name) {
      ...
      if (receiptFile.size > 1048576) {
        return new Response(JSON.stringify({ error: 'El archivo es demasiado grande. Máximo 1MB' }), { status: 400 });
      }
